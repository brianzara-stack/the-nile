...
function animateWaveform(id, pct) {
  ...
  bars.forEach((b, i) => {
    ...
    b.style.background = barPct < pct ? 'var(--accent)' : 'rgba(255,255,255,0.1)';
...
function buildWaveform(containerId, pct = 0) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    b.className = 'wv-bar';
    b.style.height = (5 + Math.random() * 38) + 'px';
    b.style.background = (i / 50 * 100) < pct ? 'var(--accent)' : 'rgba(255,255,255,0.1)';
    el.appendChild(b);
  }
}
  ...
  clips.forEach((clip, i) => {
    ...
    card.innerHTML = `
      ...
      <div class="clip-card-content">
        <div class="clip-cat-tag"><i class="ti ti-tag" style="font-size:10px"></i>${clip.cat}</div>
        <div class="clip-player">
          <div class="clip-title">${clip.title}</div>
          <div class="clip-creator-row">
            <div class="avatar" style="width:24px;height:24px;font-size:10px;background:${clip.color}">${clip.initials}</div>
            <span class="clip-creator-name">${clip.creator}</span>
            <i class="ti ti-rosette-discount-check verified-icon"></i>
            <button class="follow-pill ${followedCreators[clip.id] ? 'following' : ''}" onclick="toggleFollow('${clip.id}')">${followedCreators[clip.id] ? 'Following' : 'Follow'}</button>
          </div>
        ...
        <div class="clip-desc">${clip.desc}</div>
